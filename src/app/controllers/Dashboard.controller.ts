    ...
    static async getAllDreamsWithGoalsAndActions(req: Request, res: Response) {
        try {
            ...
            for (const dream of dreams) {
                const dreamGoals: IGoal[] = await Goal.find({ "dream._id": dream._id }).lean();
                const goalsWithActions: any[] = [];

                for (const goal of dreamGoals) {
                    const goalActions: IAction[] = await Action.find({ "goal._id": goal._id }).lean();
                    goalsWithActions.push({ ...goal, actions: goalActions });
                }

                dreamsWithGoalsAndActions.push({ ...dream, goals: goalsWithActions });
            }
